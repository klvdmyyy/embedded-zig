...
comptime {
    ...
    const has_interrupts = @hasDecl(kmain, "interrupts");
    if (has_interrupts) {
        if (@hasDecl(kmain.interrupts, "RESET"))
            @compileError("Not allowed to overload the reset vector");

        for (std.meta.declarations(kmain.interrupts)) |decl| {
    ...
    for (std.meta.fields(atmega328p.VectorTable)[1..]) |field| {
        const new_insn = if (has_interrupts) overload: {
            if (@hasDecl(kmain.interrupts, field.name)) {
                const handler = @field(kmain.interrupts, field.name);
                const calling_convention = switch (@typeInfo(@TypeOf(@field(kmain.interrupts, field.name)))) {
